...
static bool button_io_notification_enabled;
    ...
    case sl_bt_evt_gatt_server_attribute_value_id:
      if (gattdb_LED_IO == evt->data.evt_gatt_server_characteristic_status.characteristic) {
        sl_bt_gatt_server_read_attribute_value(gattdb_LED_IO,
                                               ...
                                               &recv_len,
                                               &recv_val);
        if (recv_val) sl_bt_gatt_server_write_attribute_value(gattdb_BUTTON_IO, 0, sizeof(uint8_t), 1);// Aprinde LED
        else sl_bt_gatt_server_write_attribute_value(gattdb_BUTTON_IO, 0, sizeof(uint8_t), 0);// Stinge LED
        app_log("LED= %d\r\n", recv_val);
      }
      break;
