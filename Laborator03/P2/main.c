...
#define CONFIG_EXAMPLE_FIRMWARE_UPGRADE_URL "https://192.168.245.218:5000/firmware.bin" 
#define CONFIG_VERSION_URL "https://192.168.245.218:5000/version"
...
xEventGroupWaitBits(s_event_start_ota, BIT_BTN_PRESSED, pdTRUE, pdTRUE, portMAX_DELAY);
    ...
    esp_err_t err = esp_http_client_perform(client_ver);
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "HTTP GET Status = %d, content_length = %"PRId64,
                esp_http_client_get_status_code(client_ver),
                esp_http_client_get_content_length(client_ver));
    } else {
